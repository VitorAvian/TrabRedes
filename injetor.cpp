 ...
 *		 Código do Atuador Injetor de CO2
 *			Vitor Avian nUSP: 10295392
 **/

/**
 *		Descrição:
 *			Código que simula a comunicação do atuador Injetor de CO2 com o
...
int main(int argc, char *argv[]) {
	...
	while (true) {
		...
		// Recebe a mensagem de resposta do servidor sobre o injetor ligar ou desligar
		stam = recv(sockfd, buffer_in, TAM_BUFFER, 0);
		cout<<"Resposta do servidor: "<<buffer_in<<endl;
		funcao = buffer_in[0] - '0';
		if(funcao == 1){ // Dependendo do que receber desligar ou ligar e informar o gerenciador
			cout<<"Ligando injetor de CO2..."<<endl;
			send(sockfd, "Injetor ligado.", 15, 0);
		}
		else if(funcao == 0){
			cout<<"Desligando aquecedor..."<<endl;
			send(sockfd, "Injetor desligado.", 18, 0);
		}
		else
			break;
